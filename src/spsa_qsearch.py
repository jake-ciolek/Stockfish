#!/usr/bin/env python3
"""
SPSA tuner for qsearch pruning parameters using cutechess-cli.

Targets:
  - QSearchFutilityBase    in [128, 512]
  - QSearchSeeLowerGate    in [-160, 0]
  - QSearchSeeInnerOffset  in [-64, 64]
"""
...
def build_params(args: argparse.Namespace) -> List[Param]:
    return [
        Param(
            name="QSearchFutilityBase",
            lo=128,
            hi=512,
            theta=float(args.init_futility),
            a=float(args.a_futility),
            c=float(args.c_futility),
        ),
        Param(
            name="QSearchSeeLowerGate",
            lo=-160,
            hi=0,
            theta=float(args.init_see_lower),
            a=float(args.a_see_lower),
            c=float(args.c_see_lower),
        ),
        Param(
            name="QSearchSeeInnerOffset",
            lo=-64,
            hi=64,
            theta=float(args.init_see_inner),
            a=float(args.a_see_inner),
            c=float(args.c_see_inner),
        ),
    ]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Local SPSA tuner for qsearch parameters with cutechess-cli."
    )
    ...
    parser.add_argument("--init-futility", type=int, default=351)
    parser.add_argument("--init-see-lower", type=int, default=-80)
    parser.add_argument("--init-see-inner", type=int, default=0)

    parser.add_argument(
        "--a-futility", type=float, default=10.0, help="Base a for QSearchFutilityBase."
    )
    parser.add_argument(
        "--a-see-lower",
        type=float,
        default=4.0,
        help="Base a for QSearchSeeLowerGate.",
    )
    parser.add_argument(
        "--a-see-inner",
        type=float,
        default=3.0,
        help="Base a for QSearchSeeInnerOffset.",
    )
    parser.add_argument(
        "--c-futility", type=float, default=8.0, help="Base c for QSearchFutilityBase."
    )
    parser.add_argument(
        "--c-see-lower",
        type=float,
        default=4.0,
        help="Base c for QSearchSeeLowerGate.",
    )
    parser.add_argument(
        "--c-see-inner",
        type=float,
        default=2.0,
        help="Base c for QSearchSeeInnerOffset.",
    )

    parser.add_argument(
        "--log-dir",
        default=str(Path.cwd() / "spsa_logs"),
...
def main() -> int:
    ...
    for iteration in range(start_iter + 1, start_iter + args.iterations + 1):
        ...
        record = {
            ...
            "draws": score["draws"],
            "games": score["games"],
            "theta_futility": clip_round(params[0].theta, params[0].lo, params[0].hi),
            "theta_see_lower": clip_round(params[1].theta, params[1].lo, params[1].hi),
            "theta_see_inner": clip_round(params[2].theta, params[2].lo, params[2].hi),
            "plus_futility": plus["QSearchFutilityBase"],
            "plus_see_lower": plus["QSearchSeeLowerGate"],
            "plus_see_inner": plus["QSearchSeeInnerOffset"],
            "minus_futility": minus["QSearchFutilityBase"],
            "minus_see_lower": minus["QSearchSeeLowerGate"],
            "minus_see_inner": minus["QSearchSeeInnerOffset"],
            "ak_futility": ak["QSearchFutilityBase"],
            "ak_see_lower": ak["QSearchSeeLowerGate"],
            "ak_see_inner": ak["QSearchSeeInnerOffset"],
            "ck_futility": ck["QSearchFutilityBase"],
            "ck_see_lower": ck["QSearchSeeLowerGate"],
            "ck_see_inner": ck["QSearchSeeInnerOffset"],
            "elapsed_sec": elapsed,
            "seed": iter_seed,
        }
